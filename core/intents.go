...
func NuevoClasificador() *Clasificador {
	...
	c.intentos = []Intento{
		...
		{
			Nombre:   "informe_piloto",
			Frases:   []string{"informe del piloto", "informe piloto", "cierre del piloto", "reporte del piloto", "reporte piloto", "métricas del piloto", "metricas del piloto", "cuántas horas ahorré", "cuantas horas ahorre"},
			Palabras: []string{"piloto", "informe del piloto", "cierre del piloto"},
			Handler: func(cmd string, h *Hands) string {
				informe, ruta := h.generarInformePilotoYGuardar(h.informesDir())
				if ruta == "" {
					return informe
				}
				return informe + " Guardado en " + ruta + "."
			},
		},
		{
